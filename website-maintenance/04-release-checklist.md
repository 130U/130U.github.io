...
- the four-item primary navigation appears consistently on every route;
...
- the exported runtime-asset manifest matches the approved public files;
- the exported root carries the static Content Security Policy, strict
  ...
  metadata;
- the single approved brand font remains self-hosted and licensed; particle
  word sampling remains independent of web-font timing.
...
- Push a short-lived feature branch and open a pull request against `main`.
...
- Keep release tags archive-only; they must not trigger the Pages workflow.
