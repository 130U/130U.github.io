...
const SOURCE_ICON = path.join(ROOT, "source-assets", "brand", "icon-512.png");
...
const [favicon16, favicon32, appleTouchIcon, sourceIcon] = await Promise.all([
  renderSquare(mark, 16, 0.84),
  renderSquare(mark, 32, 0.84),
  renderSquare(mark, 180, 0.74),
  renderSquare(mark, 512, 0.74),
]);
...
await Promise.all([
  ...
  writeFile(SOURCE_ICON, sourceIcon),
]);
